...
`source = hermes://agent/holocene-pm`. Inbound commands arrive through the
...
board, and route work to other agents (e.g. the `holocene-dev` role).
...
Put `repo = holocene` in event data; never insert repo or agent
...
| --- | --- | --- |
| Bank | `agent-holocene-pm` | `holocene` |
...
```bash
hindsight memory retain holocene "holocene-pm: <fact>" --context <cat>
hindsight memory recall holocene "<question>"
```
